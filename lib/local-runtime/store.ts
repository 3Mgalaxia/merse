...
const BASE_DIR = path.join(process.cwd(), ".tmp-local-runtime");
const MAX_LOG_LINES = 350;
const BRIDGE_FILE_NAME = "__merse_runtime_bridge.js";
const BRIDGE_PUBLIC_PATH = `/${BRIDGE_FILE_NAME}`;

const globalStore = globalThis as typeof globalThis & {
  __MERSE_LOCAL_RUNTIME_SESSIONS__?: Map<string, RuntimeSession>;
};

const sessions =
  globalStore.__MERSE_LOCAL_RUNTIME_SESSIONS__ ??
  (globalStore.__MERSE_LOCAL_RUNTIME_SESSIONS__ = new Map<string, RuntimeSession>());
...
function appendLog(session: RuntimeSession, line: string) {
  const text = line.trim();
  if (!text) return;
  const timestamp = new Date().toLocaleTimeString("pt-BR", { hour12: false });
  session.logs.push(`[${timestamp}] ${text}`);
...
export function sanitizeRelativePath(rawPath: string) {
  const normalized = rawPath.replace(/\\/g, "/").trim();
  const withoutRoot = normalized.replace(/^\/+/, "");
  const safePath = path.posix.normalize(withoutRoot);
  ...
  return await new Promise<number>((resolve, reject) => {
    ...
    server.listen(0, () => {
      ...
      if (!address || typeof address === "string") {
        server.close(() => reject(new Error("Não foi possível reservar porta para runtime local.")));
        return;
      }
...
async function writeProjectFiles(session: RuntimeSession, files: RuntimeFilePayload[]) {
  ...
  for (const file of files) {
    ...
    const absoluteRoot = path.resolve(session.rootDir);
    const resolvedTarget = path.resolve(absolutePath);

    if (!resolvedTarget.startsWith(absoluteRoot)) continue;
...
function getRuntimeBridgeSource() {
  return [
    "(function () {",
    ...
    "        var selectedComponentName = selectedMeta && selectedMeta.componentName ? selectedMeta.componentName : '';",
    "        var selectedSelectorHint = selectedMeta && selectedMeta.selectorHint ? selectedMeta.selectorHint : '';",
    "        var selectedTextPreview = selectedMeta && selectedMeta.textPreview ? selectedMeta.textPreview : '';",
    "",
    "        if (!selectedComponentName && !selectedSelectorHint) {",
    "          if (statusText instanceof HTMLElement) statusText.textContent = 'Selecione um componente antes de enviar.';",
    "          return;",
    "        }",
    "",
    ...
    "            selectorHint: selectedSelectorHint || null,",
    "            textPreview: selectedTextPreview || null,",
    "            attachment: null,",
    "          });",
...
async function writeBridgeScriptFile(session: RuntimeSession) {
  const publicDir = path.join(session.rootDir, "public");
  await fs.mkdir(publicDir, { recursive: true });
  await fs.writeFile(path.join(publicDir, BRIDGE_FILE_NAME), getRuntimeBridgeSource(), "utf-8");
}

function injectBridgeTag(source: string, mode: "document" | "layout" | "html") {
  if (source.includes(BRIDGE_FILE_NAME)) {
    return { changed: false, output: source };
  }

  if (mode === "document") {
    const headSelfClosing = /<Head\b([^>]*)\/>/;
    if (headSelfClosing.test(source)) {
      const output = source.replace(
        headSelfClosing,
        `<Head$1>\n        <script src="${BRIDGE_PUBLIC_PATH}" defer></script>\n      </Head>`,
      );
      return { changed: output !== source, output };
    }

    const headClosing = /<\/Head>/;
    if (headClosing.test(source)) {
      const output = source.replace(
        headClosing,
        `        <script src="${BRIDGE_PUBLIC_PATH}" defer></script>\n      </Head>`,
      );
    ...
    const nextScriptTag = /<NextScript\s*\/>/;
    if (nextScriptTag.test(source)) {
      const output = source.replace(
        nextScriptTag,
        `<script src="${BRIDGE_PUBLIC_PATH}" defer></script>\n        <NextScript />`,
      );
  ...
  if (mode === "layout") {
    ...
    const htmlClosing = /<\/html>/;
    if (htmlClosing.test(source)) {
      const output = source.replace(
        htmlClosing,
        `      <script src="${BRIDGE_PUBLIC_PATH}" defer></script>\n    </html>`,
      );
  ...
  if (mode === "html") {
    const bodyClosing = /<\/body>/i;
    if (bodyClosing.test(source)) {
      const output = source.replace(bodyClosing, `  <script src="${BRIDGE_PUBLIC_PATH}" defer></script>\n</body>`);
      return { changed: output !== source, output };
    }

    const headClosing = /<\/head>/i;
    if (headClosing.test(source)) {
      const output = source.replace(headClosing, `  <script src="${BRIDGE_PUBLIC_PATH}" defer></script>\n</head>`);
      return { changed: output !== source, output };
    }
  }
...
async function tryInjectBridgeIntoFile(
  session: RuntimeSession,
  relativePath: string,
  mode: "document" | "layout" | "html",
) {
  const absolutePath = path.join(session.rootDir, relativePath);
  try {
    const source = await fs.readFile(absolutePath, "utf-8");
    const { changed, output } = injectBridgeTag(source, mode);
    if (!changed) {
      return false;
    }
    await fs.writeFile(absolutePath, output, "utf-8");
    appendLog(session, `Bridge injetado em ${relativePath}.`);
    return true;
  } catch {
    return false;
  }
...
async function installRuntimeBridge(session: RuntimeSession) {
  await writeBridgeScriptFile(session);

  const candidates: Array<{ path: string; mode: "document" | "layout" | "html" }> = [
    { path: "pages/_document.tsx", mode: "document" },
    { path: "pages/_document.ts", mode: "document" },
    { path: "pages/_document.jsx", mode: "document" },
    { path: "pages/_document.js", mode: "document" },
    { path: "src/pages/_document.tsx", mode: "document" },
    { path: "src/pages/_document.ts", mode: "document" },
    { path: "src/pages/_document.jsx", mode: "document" },
    { path: "src/pages/_document.js", mode: "document" },
    { path: "app/layout.tsx", mode: "layout" },
    { path: "app/layout.ts", mode: "layout" },
    { path: "app/layout.jsx", mode: "layout" },
    { path: "app/layout.js", mode: "layout" },
    { path: "src/app/layout.tsx", mode: "layout" },
    { path: "src/app/layout.ts", mode: "layout" },
    { path: "src/app/layout.jsx", mode: "layout" },
    { path: "src/app/layout.js", mode: "layout" },
    { path: "index.html", mode: "html" },
    { path: "public/index.html", mode: "html" },
  ];

  for (const candidate of candidates) {
    // eslint-disable-next-line no-await-in-loop
    const injected = await tryInjectBridgeIntoFile(session, candidate.path, candidate.mode);
    if (injected) {
      appendLog(session, "Bridge de seleção visual habilitado no preview.");
      return;
    }
  }
  ...
  appendLog(
    session,
    "Bridge não pôde ser injetado automaticamente (sem entrypoint compatível). O preview continua funcionando sem seleção visual.",
  );
}
...
function spawnLogged(
  ...
  options: {
    ...
    stageOnStart?: RuntimeStage;
  },
) {
  if (options.stageOnStart) {
    session.stage = options.stageOnStart;
  }
  ...
  child.stdout.on("data", (chunk) => appendLog(session, chunk.toString()));
  child.stderr.on("data", (chunk) => appendLog(session, chunk.toString()));
  return child;
}

async function runInstallStep(session: RuntimeSession) {
  session.stage = "installing";
  const installer = spawnLogged(session, "npm", ["install", "--no-audit", "--no-fund"], {
    cwd: session.rootDir,
  });

  await new Promise<void>((resolve, reject) => {
    installer.once("error", (error) => reject(error));
    installer.once("exit", (code) => {
      if (code === 0) {
        resolve();
      } else {
        reject(new Error(`npm install finalizou com código ${code ?? "desconhecido"}.`));
      }
    });
  });
}

function attachProcessStateHandlers(session: RuntimeSession, proc: ChildProcess) {
  proc.once("error", (error) => {
    appendLog(session, `Erro no processo: ${error.message}`);
  ...
  proc.once("exit", (code) => {
    if (session.stage === "stopped") {
      appendLog(session, "Runtime encerrado pelo usuário.");
    } else if (code === 0) {
      appendLog(session, "Runtime finalizado com sucesso.");
      session.stage = "stopped";
    } else {
      const message = `Runtime finalizado com código ${code ?? "desconhecido"}.`;
...
function tryMarkReadyFromLog(session: RuntimeSession, logLine: string) {
  const line = logLine.toLowerCase();
  const hasReadyWord = /\bready\b/.test(line);
  const hasLocalUrl = /local:\s*http|localhost:\d+|127\.0\.0\.1:\d+/.test(line);
  const hasServerStarted = /started server|server started|listening on/.test(line);
  const hasFailureSignal = /error|failed|eaddrinuse|address already in use/.test(line);

  if ((hasReadyWord || hasLocalUrl || hasServerStarted) && !hasFailureSignal) {
    session.stage = "ready";
    session.updatedAt = now();
  }
}

function bootDevProcess(session: RuntimeSession) {
  session.stage = "starting";

  const proc = spawnLogged(session, "npm", ["run", "dev", "--", "--port", String(session.port)], {
    cwd: session.rootDir,
    env: {
  ...
  proc.stdout.on("data", (chunk) => tryMarkReadyFromLog(session, chunk.toString()));
  proc.stderr.on("data", (chunk) => tryMarkReadyFromLog(session, chunk.toString()));

  attachProcessStateHandlers(session, proc);
}

async function cleanupSession(session: RuntimeSession) {
  try {
...
async function startSessionBootstrap(sessionId: string, files: RuntimeFilePayload[]) {
  ...
  try {
    ...
    session.updatedAt = now();
    await writeProjectFiles(session, files);
    appendLog(session, `${files.length} arquivos recebidos.`);
    await installRuntimeBridge(session);

    await runInstallStep(session);

    bootDevProcess(session);
  } catch (error) {
...
export async function createRuntimeSession(projectName: string, files: RuntimeFilePayload[]) {
  await ensureBaseDir();

  const id = createId();
  const port = await getFreePort();
  const rootDir = path.join(BASE_DIR, id);
  ...
  const session: RuntimeSession = {
    ...
    updatedAt: now(),
    proc: null,
    componentHints: new Map<string, string>(),
  };

  appendLog(session, `Sessão criada para ${projectName}.`);

  sessions.set(id, session);
  void startSessionBootstrap(id, files);
...
export function getRuntimeComponentHint(sessionId: string, componentName: string) {
  const session = sessions.get(sessionId);
  if (!session) return null;
  const key = normalizeComponentKey(componentName);
  if (!key) return null;
  return session.componentHints.get(key) ?? null;
}

export function setRuntimeComponentHint(sessionId: string, componentName: string, filePath: string) {
  const session = sessions.get(sessionId);
  if (!session) return;
  const key = normalizeComponentKey(componentName);
  if (!key || !filePath.trim()) return;
  session.componentHints.set(key, filePath.trim());
  session.updatedAt = now();
}
